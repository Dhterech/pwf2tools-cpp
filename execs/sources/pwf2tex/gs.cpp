...
int CalcGSIndex24(int tbp, int tbw, int x, int y) {
    calchdr;

    // PSMCT24 uses the same page/block dimensions as PSMCT32 (64x32 page, 8x8 block)
    CalcGSIndices(64, 32, tbw, block32, 8, 8, 8, 2);
    int cw = columnWord32[cx + (cy * 8)];
    return cgi(cw);
}
...
#define t(p, f)                        \
case p:                              \
    f(tbp, tbw, tx, ty, tw, th, data); \
    break;

#define psmerror(name, psm) \
fprintf(stderr, "%s: Unknown PSM Format: 0x%x\n", #name, psm);

bool WriteTexture(int psm, gsargs) {
    switch (psm) {
        t(GS_TEX_32, WriteTexture32);
        t(GS_TEX_24, WriteTexture24);
...
void WriteTexture24(gsargs) {
    gshdr(u32); // In GS memory, 24-bit pixels are stored in 32-bit words
    gsloop {
        gswrite(u32, CalcGSIndex24(tbp, tbw, x, y), *src);
        src++;
    }
}

void ReadTexture24(gsargs) {
    gshdr(u32); // In GS memory, 24-bit pixels are read in 32-bit words
    gsloop {
        *src = gsread(u32, CalcGSIndex24(tbp, tbw, x, y));
        src++;
    }
}
